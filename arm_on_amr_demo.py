...
# Model of the pose
model = [
        (xpid_1, (0.5455309152603149, 0.030417228117585182, 0.03022703155875206)),
        (xpid_2, (0.4135916233062744, -0.08103403449058533, 0.03208526596426964)),
        (xpid_3, (0.4067361652851105, 0.10923030972480774, 0.03284527733922005))
    ]
...
# The parameters of the arm for servoJ
velocity = 0.25
acceleration = 0.5
dt = 0.1  #2ms
lookahead_time = 0.2
...
ur.setTcp([0.0, 0.0, 0.180, 0.0, 0.0, 0.0])
...
def frame_callback(data):
    ...
    if data.position_error_flags != 0 or \
        data.pose_error_flags != 0:
        # print(f'error flag: {data.position_error_flags}, {data.pose_error_flags}')
        ur_base_updated = False
...
def point_callback(data):
    ...
    if data.error_flags != 0:
        # print(f'transpoder error: {data.error_flags}')
        transponder_updated = False
...
if __name__ == '__main__':
    ...
    start_time = 0
    stop_time = 0
    transponder_in_urFrame_converted = [0, 0, 0, 0, 0, 0, 0]
    dt = 2

    # Moving the MiR to the starting point
    Mir_Control.go_to_location('starting_point')

    # Moving the MiR to the table side
    Mir_Control.go_to_location('table_side')

    # tracking the transponder
    z_offset = 0.15
    ...
    # Moving the MiR to the drop site
    Mir_Control.go_to_location('drop_site')

    print('Dropping the transponder')

    drop_position_1 = [-0.03787091374397278, 0.784774899482727, 0.058657024055719376, -2.109898328781128, 2.27685284614563, 0.09694818407297134]
    drop_position_2 = [-0.05462879315018654, 0.7991215586662292, -0.38030776381492615, -2.1308059692382812, 2.2391738891601562, 0.11764967441558838]
    ...
    '''Add action to move to starting point'''
